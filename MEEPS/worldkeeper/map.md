...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S28 at `f8cd35ce` is current canon; site pin `7b7b8ed8`, deploy run `31507273979`, and exact public bytes complete custody. World main still equals the blessing. S28 crossed on the third attempt: Draig first repaired his containment edge, then Wright repaired the sweep's move-on-delivery replay edge; a later full candidate was discarded on one town mail race before the stable freeze landed nine admissions and nineteen rewritten sketchbooks atomically. The queue remainder is four; Caelum Reeves, Claran, Drift, and Lassi need founder/Illuminator judgment rather than more arithmetic. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
